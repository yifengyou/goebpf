...
const (
	...
	BitLongFile                                            //完整文件名称 /home/go/src/tsc/server.go
...
// 开启调试
var DebugFlag = flag.Bool("debug", true, "Enable debug mode or not. default not.")
...
func Newlog(out io.Writer, prefix string, flag int) *logger {
	...
	log := &logger{out: out, prefix: prefix, flag: flag, file: nil, debugClose: !*DebugFlag, calldDepth: 2}
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						//找到最后一个'/'之后的文件名称  如:/home/go/src/tsc.go 得到 "tsc.go"
						short = file[i+1:]
						break
					}
				}
